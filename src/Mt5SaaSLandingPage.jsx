...
function ProofCard({ item, index, onOpen }) {
  const [imgError, setImgError] = useState(false);
  const canPreview = !imgError;

  return (
    <Card className="group overflow-hidden rounded-[28px] border-slate-200/90 bg-white/90 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-slate-300 hover:shadow-xl hover:shadow-slate-200/60">
      <button
        type="button"
        onClick={() => canPreview && onOpen(index)}
        disabled={!canPreview}
        aria-label={
          canPreview
            ? `Open full result image for ${item.title}`
            : `${item.title} image is unavailable`
        }
      ...
      >
        {!imgError ? (
          <>
            ...
            <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(120deg,transparent_20%,rgba(255,255,255,0.24)_50%,transparent_80%)] opacity-70" />
            <div className="absolute bottom-4 left-4 inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/90 px-4 py-2 text-xs font-semibold text-slate-900 shadow-lg shadow-slate-900/10 backdrop-blur">
              <Expand className="h-3.5 w-3.5" />
              Click to expand
            </div>
          </>
        ) : (
          <div className="flex h-64 flex-col items-center justify-center gap-3 bg-gradient-to-br from-slate-100 to-slate-50 px-6 text-center text-sm text-slate-500">
      ...
      <CardContent className="space-y-3 p-6">
        ...
        {canPreview ? (
          <button
            ...
            className="inline-flex w-fit items-center gap-2 text-sm font-semibold text-slate-900 transition hover:text-slate-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300"
          >
            View full image
            <Expand className="h-3.5 w-3.5" />
          </button>
...
export default function Mt5SaasLandingPage() {
  ...
  const activeProof =
    activeProofIndex === null ? null : proofImages[activeProofIndex];
    ...
    function handleKeyDown(event) {
      ...
      if (event.key === "ArrowRight") {
        setActiveProofIndex((current) =>
          current === null ? 0 : (current + 1) % proofImages.length
        );
      }
      ...
      if (event.key === "ArrowLeft") {
        setActiveProofIndex((current) =>
          current === null
            ...
            : (current - 1 + proofImages.length) % proofImages.length
        );
      }
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-100 text-slate-900">
      ...
      <main>
        ...
        <section
          id="proof"
          className="border-y border-slate-200 bg-[linear-gradient(to_bottom,rgba(255,255,255,0.9),rgba(248,250,252,0.98))]"
        >
          <div className="mx-auto max-w-7xl px-4 py-16 md:px-6">
            <SectionTitle
              eyebrow="Proof / results"
              title="Make your results easy to inspect and easy to trust"
              text="Real screenshots and proof cards help prospects believe the service faster. Each image can now be opened in a premium fullscreen preview."
            />
      ...
      <AnimatePresence>
        {activeProof ? (
          <motion.div
            className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-md md:p-6"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={closeProof}
          >
            <motion.div
              className="relative grid max-h-[calc(100vh-2rem)] w-full max-w-6xl overflow-hidden rounded-[32px] border border-white/40 bg-white/95 shadow-2xl shadow-slate-950/30 lg:grid-cols-[minmax(0,1.3fr)_360px]"
            ...
            >
              <button
                ...
                className="absolute right-3 top-3 z-10 inline-flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 bg-white/90 text-slate-900 shadow-lg shadow-slate-900/10 backdrop-blur transition hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300"
              >
                <X className="h-4 w-4" />
              </button>

              <div className="flex min-h-[320px] items-center justify-center bg-[radial-gradient(circle_at_top_right,rgba(99,102,241,0.14),transparent_28%),radial-gradient(circle_at_bottom_left,rgba(16,185,129,0.12),transparent_24%),linear-gradient(135deg,#e2e8f0,#f8fafc)] p-4 md:p-6">
                <img
                  src={activeProof.src}
                  alt={activeProof.title}
                  className="max-h-[52vh] w-full rounded-[24px] bg-white object-contain shadow-xl shadow-slate-900/10 md:max-h-[72vh]"
                />
              </div>

              <div className="flex flex-col border-t border-slate-200 bg-white/90 p-6 lg:border-l lg:border-t-0 lg:p-8">
                <Badge
                  variant="secondary"
                ...
                <p className="mt-5 text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">
                  {String(activeProofIndex + 1).padStart(2, "0")} /{" "}
                ...
                <p className="mt-auto pt-6 text-xs leading-6 text-slate-400">
